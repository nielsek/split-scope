...
subnets.each do |subnet|
  ...
  subnet[3] - dhcpware/splitpercent - eg dhcpd/60,kea/30,dhcpd/10 (split between 3 hosts 60%,30%,10%)
  ...
  subnet[3].split(',').each_with_index do |hostinfo, index|
    type = hostinfo.split('/')[0]
    prcnt = hostinfo.split('/')[1]

    # Config pr. host / pr. ip version
    configarr = "#{index}-#{type}-v#{cidr.version}"
